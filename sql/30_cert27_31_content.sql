...
SELECT id, 'Secondary Market Fundamentals for Lenders', 1, 'approved' FROM certifications WHERE cert_number = 27
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Why Secondary Markets Matter for Community Lenders', 'why-secondary-markets-matter-for-community-lenders',
...
WHERE c.cert_number = 27 AND m.sort_order = 1
ON CONFLICT DO NOTHING;



INSERT INTO quizzes (certification_id, title, passing_score, time_limit_minutes, status)
SELECT id, 'Secondary Market & Mortgage Liquidity Assessment', 75, 40, 'approved' FROM certifications WHERE cert_number = 27
ON CONFLICT DO NOTHING;


WITH qid AS (
  SELECT q.id AS quiz_id FROM quizzes q JOIN certifications c ON q.certification_id = c.id
  WHERE c.cert_number = 27 AND q.title = 'Secondary Market & Mortgage Liquidity Assessment')
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'CDFI Certification & Treasury Programs', 1, 'approved' FROM certifications WHERE cert_number = 28
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'What is a CDFI and How Do You Get Certified?', 'what-is-a-cdfi-and-how-do-you-get-certified',
...
$BODY$, 1, 10, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 28 AND m.sort_order = 1
ON CONFLICT DO NOTHING;

INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'CDFI Fund Programs: FA, TA, Capital Magnet Fund & NMTC', 'cdfi-fund-programs-fa-ta-capital-magnet-fund-nmtc',
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'State Small Business Credit Initiative (SSBCI)', 1, 'approved' FROM certifications WHERE cert_number = 29
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'SSBCI Overview: State-Administered Capital Access Programs', 'ssbci-overview-state-administered-capital-access-programs',
...
$BODY$, 1, 9, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 29 AND m.sort_order = 1
ON CONFLICT DO NOTHING;



INSERT INTO quizzes (certification_id, title, passing_score, time_limit_minutes, status)
SELECT id, 'SSBCI, State & Local Capital Access Assessment', 75, 45, 'approved' FROM certifications WHERE cert_number = 29
ON CONFLICT DO NOTHING;


WITH qid AS (
  SELECT q.id AS quiz_id FROM quizzes q JOIN certifications c ON q.certification_id = c.id
  WHERE c.cert_number = 29 AND q.title = 'SSBCI, State & Local Capital Access Assessment')
INSERT INTO quiz_questions (quiz_id, question_text, options, correct_option_id, explanation, sort_order)
SELECT qid.quiz_id, v.question_text, v.options::jsonb, v.correct_option_id, v.explanation, v.sort_order
FROM qid CROSS JOIN (VALUES
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'EDA Economic Development Finance & RLF Structure', 1, 'approved' FROM certifications WHERE cert_number = 30
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'EDA RLF: Purpose, Eligible Recipients, and Program Requirements', 'eda-rlf-purpose-eligible-recipients-and-program-requirements',
...
$BODY$, 1, 10, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 30 AND m.sort_order = 1
ON CONFLICT DO NOTHING;

INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Operating an EDA RLF: Portfolio Management, Compliance & Sustainability', 'operating-an-eda-rlf-portfolio-management-compliance-sustainability',
...
INSERT INTO quizzes (certification_id, title, passing_score, time_limit_minutes, status)
SELECT id, 'EDA Revolving Loan Fund Assessment', 80, 45, 'approved' FROM certifications WHERE cert_number = 30
ON CONFLICT DO NOTHING;


WITH qid AS (
  SELECT q.id AS quiz_id FROM quizzes q JOIN certifications c ON q.certification_id = c.id
  WHERE c.cert_number = 30 AND q.title = 'EDA Revolving Loan Fund Assessment')
INSERT INTO quiz_questions (quiz_id, question_text, options, correct_option_id, explanation, sort_order)
SELECT qid.quiz_id, v.question_text, v.options::jsonb, v.correct_option_id, v.explanation, v.sort_order
FROM qid CROSS JOIN (VALUES
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'Environmental Finance: SRFs, Brownfields & WIFIA', 1, 'approved' FROM certifications WHERE cert_number = 31
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Clean Water and Drinking Water State Revolving Funds', 'clean-water-and-drinking-water-state-revolving-funds',
...
$BODY$, 1, 9, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 31 AND m.sort_order = 1
ON CONFLICT DO NOTHING;

INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'EPA Brownfields RLF & Environmental Underwriting', 'epa-brownfields-rlf-environmental-underwriting',
