...
SELECT id, 'Rural Infrastructure Finance: USDA, DOE & Broadband', 1, 'approved' FROM certifications WHERE cert_number = 32
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'USDA Community Facilities, Rural Electric & Broadband Programs', 'usda-community-facilities-rural-electric-broadband-programs',
...
WHERE c.cert_number = 32 AND m.sort_order = 1
ON CONFLICT DO NOTHING;

INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'DOE Energy Programs, DOT TIFIA/RRIF & Infrastructure Capital Stacks', 'doe-energy-programs-dot-tifiarrif-infrastructure-capital-stacks',
...
SELECT id, 'Infrastructure, Energy, Transportation & Utility Finance Assessment', 75, 45, 'approved' FROM certifications WHERE cert_number = 32
ON CONFLICT DO NOTHING;


WITH qid AS (
  SELECT q.id AS quiz_id FROM quizzes q JOIN certifications c ON q.certification_id = c.id
  WHERE c.cert_number = 32 AND q.title = 'Infrastructure, Energy, Transportation & Utility Finance Assessment')
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'Export Finance Programs: EXIM & SBA', 1, 'approved' FROM certifications WHERE cert_number = 33
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'EXIM Bank & SBA Export Programs: Tools for Small Business Exporters', 'exim-bank-sba-export-programs-tools-for-small-business-exporters',
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'Tribal Capital Access: BIA, Native CDFIs & Sovereignty', 1, 'approved' FROM certifications WHERE cert_number = 34
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'BIA Indian Loan Guarantee and Native CDFI Programs', 'bia-indian-loan-guarantee-and-native-cdfi-programs',
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'Federal Student Loan Programs: History and Awareness', 1, 'approved' FROM certifications WHERE cert_number = 35
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Why Federal Student Lending Is Not a New Private Lender Pathway', 'why-federal-student-lending-is-not-a-new-private-lender-pathway',
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'Capital Stack Fundamentals', 1, 'approved' FROM certifications WHERE cert_number = 36
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Understanding the Capital Stack: Grant, Debt, Equity & Guarantees', 'understanding-the-capital-stack-grant-debt-equity-guarantees',
...
$BODY$, 1, 10, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 36 AND m.sort_order = 1
ON CONFLICT DO NOTHING;

INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Partner Pitch Strategy & Building a Funding Source Matrix', 'partner-pitch-strategy-building-a-funding-source-matrix',
