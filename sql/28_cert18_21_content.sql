...
SELECT id, 'What It Means to Become a Lender', 1, 'approved' FROM certifications WHERE cert_number = 18
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'The Decision to Lend: Responsibilities and Realities', 'the-decision-to-lend-responsibilities-and-realities',
...
WHERE c.cert_number = 18 AND m.sort_order = 1
ON CONFLICT DO NOTHING;

INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'The Five Major Government Lending Models', 'the-five-major-government-lending-models',
...
-- Module 2
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'Choosing the Right Lender Model for Your Organization', 2, 'approved' FROM certifications WHERE cert_number = 18
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Matching Your Mission to a Lending Program', 'matching-your-mission-to-a-lending-program',
...
$BODY$, 1, 7, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 18 AND m.sort_order = 2
ON CONFLICT DO NOTHING;
...
SELECT id, 'Government Lending Models Foundations Assessment', 75, 40, 'approved' FROM certifications WHERE cert_number = 18
ON CONFLICT DO NOTHING;


WITH qid AS (
  SELECT q.id AS quiz_id FROM quizzes q JOIN certifications c ON q.certification_id = c.id
  WHERE c.cert_number = 18 AND q.title = 'Government Lending Models Foundations Assessment')
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'Choosing the Right Lending Entity Structure', 1, 'approved' FROM certifications WHERE cert_number = 19
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Nonprofit Loan Fund vs. CDFI vs. For-Profit Lender', 'nonprofit-loan-fund-vs-cdfi-vs-for-profit-lender',
...
$BODY$, 1, 8, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 19 AND m.sort_order = 1
ON CONFLICT DO NOTHING;

INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'State Lending Licenses, NMLS, and SAM.gov Registration', 'state-lending-licenses-nmls-and-samgov-registration',
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'BSA, AML, OFAC, Fair Lending & Loan Committee Governance', 2, 'approved' FROM certifications WHERE cert_number = 19
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Bank Secrecy Act, Anti-Money Laundering & OFAC Screening', 'bank-secrecy-act-anti-money-laundering-ofac-screening',
...
$BODY$, 1, 9, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 19 AND m.sort_order = 2
ON CONFLICT DO NOTHING;
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'The SBA 7(a) Lending Ecosystem', 1, 'approved' FROM certifications WHERE cert_number = 20
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'SBA 7(a) Overview: Programs, Eligibility, and Lender Types', 'sba-7a-overview-programs-eligibility-and-lender-types',
...
$BODY$, 1, 10, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 20 AND m.sort_order = 1
ON CONFLICT DO NOTHING;


INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'SBA Express, Export Programs & CAPLines', 2, 'approved' FROM certifications WHERE cert_number = 20
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'Partnering with SBA Lenders: Strategy for Community Organizations', 'partnering-with-sba-lenders-strategy-for-community-organizations',
...
$BODY$, 1, 8, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 20 AND m.sort_order = 2
ON CONFLICT DO NOTHING;
...
INSERT INTO quizzes (certification_id, title, passing_score, time_limit_minutes, status)
SELECT id, 'SBA 7(a) & Small Business Lending Assessment', 75, 50, 'approved' FROM certifications WHERE cert_number = 20
ON CONFLICT DO NOTHING;


WITH qid AS (
  SELECT q.id AS quiz_id FROM quizzes q JOIN certifications c ON q.certification_id = c.id
  WHERE c.cert_number = 20 AND q.title = 'SBA 7(a) & Small Business Lending Assessment')
INSERT INTO quiz_questions (quiz_id, question_text, options, correct_option_id, explanation, sort_order)
SELECT qid.quiz_id, v.question_text, v.options::jsonb, v.correct_option_id, v.explanation, v.sort_order
FROM qid CROSS JOIN (VALUES
  ('What is the SBA guarantee percentage for a standard 7(a) loan over $150,000?', '[{"id":"a","text":"85%"},{"id":"b","text":"75%"},{"id":"c","text":"90%"},{"id":"d","text":"50%"}]', 'b', 'SBA guarantees up to 85% for loans up to $150,000 and 75% for loans above that threshold.', 1),
  ('Which SBA 7(a) product offers approval within 36 hours and a 50% guarantee?', '[{"id":"a","text":"Standard 7(a)"},{"id":"b","text":"SBA Express"},{"id":"c","text":"Export Express"},{"id":"d","text":"CAPLine"}]', 'b', 'SBA Express offers a 36-hour turnaround and a 50% guarantee, allowing lenders to use their own underwriting processes for loans up to $500,000.', 2),
  ('What is the maximum loan amount under the SBA 7(a) program?', '[{"id":"a","text":"$2 million"},{"id":"b","text":"$3.5 million"},{"id":"c","text":"$5 million"},{"id":"d","text":"$10 million"}]', 'c', 'The SBA 7(a) program has a maximum loan amount of $5 million.', 3),
  ('Which SBA lender type is specifically designed for nonprofits and CDFIs?', '[{"id":"a","text":"SBA Express lender"},{"id":"b","text":"Community Advantage SBLC"},{"id":"c","text":"Preferred Lender Program (PLP)"},{"id":"d","text":"Certified Lender Program (CLP)"}]', 'b', 'Community Advantage Small Business Lending Companies are a licensed class of SBA 7(a) lender specifically designed for nonprofits, CDFIs, and mission-driven organizations.', 4),
...
INSERT INTO modules (certification_id, title, sort_order, status)
SELECT id, 'The SBA Microloan Program: Intermediary Model', 1, 'approved' FROM certifications WHERE cert_number = 21
ON CONFLICT DO NOTHING;


INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'SBA Microloan Program: How Intermediaries Work', 'sba-microloan-program-how-intermediaries-work',
...
$BODY$, 1, 9, 'approved'
FROM modules m JOIN certifications c ON m.certification_id = c.id
WHERE c.cert_number = 21 AND m.sort_order = 1
ON CONFLICT DO NOTHING;

INSERT INTO lessons (module_id, title, slug, content, sort_order, read_time_minutes, status)
SELECT m.id, 'CDC/504 Program: Fixed-Rate Project Finance for Small Business', 'cdc504-program-fixed-rate-project-finance-for-small-business',
